...
describe('EncryptDialogComponent', () => {
  ...
  const mockTranslations: Record<string, string> = {
    ...
    'TOASTS.ENCRYPT.SUCCESS_MESSAGE': 'Encryption successful',
    'TOASTS.ENCRYPT.ERROR_MESSAGE': 'Encryption failed',
    'TOASTS.ENCRYPT.STARTED_MESSAGE': 'Encryption started',
    'TOASTS.ENCRYPT.FILES_DONE_MESSAGE': 'Files processed',
    'TOASTS.ENCRYPT.FILES_FAILED_MESSAGE': 'Files failed',
    'TOASTS.ENCRYPT.FAILED_FILES_MESSAGE': 'Failed files'
  };
  ...
  beforeEach(async () => {
    ...
    dialogService = jasmine.createSpyObj('DialogService', ['hideEncryptDialog'], {
      encryptDialogVisible$: new BehaviorSubject<boolean>(false)
    });

    messageService = jasmine.createSpyObj('MessageService', ['add']);

    fileEncryptionService = jasmine.createSpyObj('FileEncryptionService', ['encryptFiles'], {
  ...
  describe('Dialog initialization', () => {
    it('should initialize with default values', () => {
      ...
      expect(component.deleteAfter).toBeFalse();
      expect(component.encryptDialogVisible).toBeFalse();
      expect(component.loading).toBeFalse();
    });

    it('should subscribe to dialog visibility changes', () => {
      (dialogService.encryptDialogVisible$ as BehaviorSubject<boolean>).next(true);
      fixture.detectChanges();
      expect(component.encryptDialogVisible).toBeTrue();
    });
  ...
  describe('Key selection', () => {
    it('should handle public key selection', async () => {
    ...
    it('should not update keyPath for private key selection', async () => {
  ...
  describe('Encryption process', () => {
    const setupEncryption = () => {
      ...
      component.keyPath = '/path/to/public.key';
      component.deleteAfter = true;
      component.doArchive = false;
    };
    ...
    it('should start encryption process with correct parameters', () => {
      fileEncryptionService.encryptFiles.and.returnValue(Promise.resolve({
        okCount: 1,
        failCount: 0,
        failedFiles: []
      }));
      ...
      expect(fileEncryptionService.encryptFiles).toHaveBeenCalledWith('testPassword', '/path/to/public.key', true, false);
    });

    it('should show and hide loading indicator during encryption', async () => {
      fileEncryptionService.encryptFiles.and.returnValue(Promise.resolve({
        okCount: 1,
        failCount: 0,
        failedFiles: []
    ...
    it('should handle mixed success/failure results', async () => {
      ...
      fileEncryptionService.encryptFiles.and.returnValue(Promise.resolve(result));
  ...
  describe('Translation handling', () => {
    it('should translate success messages correctly', async () => {
      ...
      expect(messageService.add).toHaveBeenCalledWith({
        ...
        detail: mockTranslations['TOASTS.ENCRYPT.SUCCESS_MESSAGE']
      });
    });
    ...
    it('should translate error messages correctly', async () => {
      ...
      expect(messageService.add).toHaveBeenCalledWith({
        ...
        detail: mockTranslations['TOASTS.ENCRYPT.ERROR_MESSAGE']
      });
    });
  });
});
